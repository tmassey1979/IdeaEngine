...
- A new C# backend foundation now lives under `backend/` and currently covers shared self-build contracts, backlog indexing, bounded developer-operation planning, local queue persistence, workflow-state tracking including active recovery children, per-issue execution records, GitHub backlog discovery, stronger review/test execution, repeated-failure quarantine, long-stall quarantine sweeps, quarantined-work remediation signaling and recovery-story creation on GitHub, recovery-aware scheduling with source-issue linkage, stage-aware in-progress heartbeat comments with timing, stalled-state, and recovery-release data, automatic workflow-label transitions, and validated workflow sync with a guard that only updates GitHub after the workflow reaches `validated`.
