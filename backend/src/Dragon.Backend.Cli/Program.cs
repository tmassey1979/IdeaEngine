...
static int RunConfigGenerateKey()
{
    Console.WriteLine(ConfigurationEncryptionService.GenerateEncodedKey());
    return 0;
}
...
static int RunAgentConfigSet(IReadOnlyDictionary<string, string> options)
{
    ...
    {
        Console.Error.WriteLine("Missing required option: --agent");
        return 1;
    }
...
static int RunAgentConfigList(IReadOnlyDictionary<string, string> options)
{
    try
    {
        var resolver = CreateConfigurationResolver(options);
        PrintJson(resolver.ListStoredAgents());
        return 0;
    }
    catch (InvalidOperationException exception)
    {
...
static int RunAgentConfigResolve(IReadOnlyDictionary<string, string> options)
{
    try
    {
        var resolver = CreateConfigurationResolver(options);
        var agentName = GetNullable(options, "agent");
...
static int RunPolling(IReadOnlyDictionary<string, string> options)
{
    ...
    ExportStartupStatusIfRequested(
        loop,
        root,
        options,
        "run-polling",
...
static int RunGithubRunPolling(IReadOnlyDictionary<string, string> options)
{
    ...
    ExportStartupStatusIfRequested(
        ...
        maxPasses: maxPasses,
        passBudgetRemaining: maxPasses);
    var githubStatusExporter = CreateGithubStatusExporter(loop, root, options, "github-run-polling", "complete", maxPasses: maxPasses);
    var result = loop.RunPollingFromGithub(
        ...
        idlePassesBeforeStop: GetInt(options, "idle-passes", 2),
        maxCyclesPerPass: GetInt(options, "max-cycles", 100),
        passCompleted: githubStatusExporter
    );
    PrintJson(result);
    return 0;
}
...
static int RunGithubRunWatch(IReadOnlyDictionary<string, string> options)
{
    ...
    ExportStartupStatusIfRequested(
        ...
        maxPasses: maxPasses,
        passBudgetRemaining: maxPasses);
    var statusExporter = CreateGithubStatusExporter(
        loop,
        root,
        options,
        "github-run-watch",
...
static void ExportStartupStatusIfRequested(
    SelfBuildLoop loop,
    string root,
    IReadOnlyDictionary<string, string> options,
    string source,
...
static Action<int, RunUntilIdleResult, LatestGithubReplaySnapshot?>? CreateGithubStatusExporter(
    SelfBuildLoop loop,
    string root,
    IReadOnlyDictionary<string, string> options,
    string source,
    ...
    string? initialWorkerCompletionReason = null)
{
    ...
    var resolvedOutputPath = Path.GetFullPath(outputPath, root);
    return (passNumber, result, latestReplay) =>
    {
        var latestPass = initialLatestPass ?? SelfBuildLoop.BuildLatestPassSummary(passNumber, result, latestReplay);
        var workerStatus = workerStatusResolver?.Invoke(passNumber, result) ?? (defaultWorkerState, (DateTimeOffset?)null, initialWorkerCompletionReason);
        var snapshot = loop.ReadStatus(
            source,
            source,
...
static AgentRuntimeConfigurationResolver CreateConfigurationResolver(IReadOnlyDictionary<string, string> options)
{
    var root = Path.GetFullPath(GetString(options, "root", Directory.GetCurrentDirectory()));
    ...
    return AgentRuntimeConfigurationResolver.CreateDefault(root, Environment.GetEnvironmentVariable, runtimeOverrides);
}
